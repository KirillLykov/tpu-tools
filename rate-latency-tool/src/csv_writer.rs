...
pub async fn run_csv_writer(
    ...
    tracker.spawn({
        ...
        async move {
            let main_loop = async move {
                ...
                loop {
                    let received = block_tx_receiver.recv_many(&mut complete_records, 16).await;
    ...
    tracker.spawn(async move {
        let mut complete_records = Vec::with_capacity(128);
        loop {
            let received = writer_receiver.recv_many(&mut complete_records, 128).await;
            if received == 0 {
