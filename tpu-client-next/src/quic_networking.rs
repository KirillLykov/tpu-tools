...
use {
    ...
    solana_quic_definitions::{QUIC_MAX_TIMEOUT, QUIC_SEND_FAIRNESS},
...
pub(crate) async fn send_data_over_stream(
    ...
    let address: std::net::SocketAddr = connection.remote_address();
    tokio::spawn(async move {
        let mut measure = Measure::start("wait for feedback");
        send_stream.finish().ok();
        match send_stream.stopped().await {
            Ok(Some(code)) => {
                // Peer stopped the stream, no reason it should do it.
                warn!(
                    "{} unexpectedly stopped the stream with code {}",
                    address, code
                );
            }
            Ok(None) => {
                // Peer acknowledged reception
                debug!("Successfully sent tx to {}", address);
            }
            Err(_) => {
                warn!("Error waiting for reception of tx by {}", address);
            }
        }
        measure.stop();
        debug!("Time taken to send data over quic stream: {}", measure);
    });
